  ...
  (async () => {
    ...
    client.on("message", async (msg) => {
      if (process.env.PREFIX) {
        ...
        try {
          const com = await client.commands?.get(command);
          com?.default.execute({ client, msg, args });

          const alias = await client.aliases?.get(command);
          alias?.default.execute({ client, msg, args });
          if (!com && !alias) {
